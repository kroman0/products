...
def installSkin(portal, pp_up, out):
    ...
    # Add subfolder path to the list of layers after "custom" layer if qMultipleSlot product isn't installed
    has_qMultipleSLots = 'qmultipleslots' in new_path
    if not has_qMultipleSLots and GENERATED_SUBFOLDER in skinsTool[layer_skin_name].objectIds():
        skin_subfolder_layer = "%%s/%%s" %% (layer_skin_name, GENERATED_SUBFOLDER)
        try:
            new_path.insert(new_path.index('custom')+1, skin_subfolder_layer)
        except ValueError:
            new_path.append(skin_subfolder_layer)
...
def uninstallSkin(skinsTool, actual_skin_name, initial_skin):
    ...
    for skin_name in skin_names:
        ...
        # remove layer with generated main_template.pt
        skin_subfolder_layer = "%%s/%%s" %% (skin_layer, GENERATED_SUBFOLDER)
        if skin_subfolder_layer in path:

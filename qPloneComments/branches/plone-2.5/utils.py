...
def send_email(reply, context, state):
    def send(context, template, args):
        msg = getattr(context, template)(**args)
        return context.MailHost.send(msg)

    def getEmail(obj, context):
        userid = obj.getProperty('userid', '')
        if userid == 'anonym':
            return obj.getProperty('email', '')
        else:
            return context.portal_membership.getMemberById(userid).getProperty('email', '')
    ...
    def getParentOwnerEmail(reply, context):
        creator_id = getParent(reply).Creator()
    ...
    if args:
        return send(context, template, args)
    else:
        return 1

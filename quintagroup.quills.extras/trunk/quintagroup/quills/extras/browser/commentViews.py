...
class ManageCommentsView(FormControllerView):
    ...
    def __init__(self, context, request):
        ...
        if self.mode in ['delete', 'display', 'update']:
            ...
            if self.subject:
                ...
                self.filtered = True
            if self.review_state == 'private':
	            self.contentFilter['review_state'] = 'private'
	            self.filtered = True
    ...
    def control(self):
        ...
        if self.mode == "update":
			...
			discussion_tool = getToolByName(self.context, 'portal_discussion')
	        for path in self.selected_comments:
		    	self. publishReply(discussion_tool, self.portal_catalog, path)
    ...
    def publishReply(self, dtool, pcatalog, path):
	    """"""
	    pass

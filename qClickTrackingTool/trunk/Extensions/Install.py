...
def convert_objects(self, container, new_type, out):
    ...
    obj_type = getObjectsType(self, container, out)
    if (obj_type and obj_type == 'Link') or (obj_type and obj_type == 'ATLink'):
...
def setPortalFactoryType(self, out,):
    ...
    if TYPE_NAME not in ftypes:
        ...
        print >>out, " %s type already  in portal_factory..." %TYPE_TITLE

    print >> out, "Set %s type to portal_factory" %TYPE_TITLE
